...
int mtk_set_gpio_dir(unsigned short gpio_nr, unsigned short gpio_dir_out)
{
	...
	if (gpio_nr >= 0 && gpio_nr <= 3) {
		...
		/* GPIOMODE2 wled */
		shift = 0;
	}/* else if (gpio_nr >= 42 && gpio_nr <= 44) { /* wled & p0_led & p1_led
		shift = 
		
		reg = ra_inl(RT2880_GPIOMODE2_REG);
		reg &= ~((msk << (((44-gpio_nr)<<1)+16)) | (msk << ((44-gpio_nr)<<1)));
		reg |= ((val << (((44-gpio_nr)<<1)+16)) | (val << ((44-gpio_nr)<<1)));
...
void gpio_init(void)
{
	...
	/* prepare alert LED(s) */
#if defined(GPIO_LED_ALERT1) && defined(GPIO_LED_INIT1) && defined(GPIO_LED_POWER)
...
{
#if defined(GPIO_USB_POWER) && defined(GPIO_USB_POWER2)
#if (GPIO_USB_POWER >= 0)
...
#if defined(GPIO_LED_ALERT1) && defined(GPIO_LED_INIT1)
#if ((GPIO_LED_ALERT1 >= 0) && (GPIO_LED_ALERT1 != GPIO_LED_INIT1) && (GPIO_LED_ALERT1 != GPIO_LED_POWER))
...
{
#if defined(GPIO_LED_ALERT1) && defined(GPIO_LED_POWER)
#if ((GPIO_LED_ALERT1 >= 0) && (GPIO_LED_ALERT1 != GPIO_LED_POWER))
	mtk_set_gpio_pin(GPIO_LED_ALERT1, GPIO_VAL_LED_HIDE);
#endif
#endif

#if defined(GPIO_LED_ALERT2)
#if (GPIO_LED_ALERT2 >= 0)
	mtk_set_gpio_pin(GPIO_LED_ALERT2, GPIO_VAL_LED_HIDE);
#endif
#endif

#if defined(GPIO_LED_ALERT3)
#if (GPIO_LED_ALERT3 >= 0)
	mtk_set_gpio_pin(GPIO_LED_ALERT3, GPIO_VAL_LED_HIDE);
#endif
#endif

#if defined(GPIO_LED_ALERT4)
...
{
#if defined(GPIO_LED_ALERT1) && defined(GPIO_LED_ALERT2) && defined(GPIO_LED_ALERT3) && defined(GPIO_LED_ALERT4) && defined(GPIO_LED_POWER)
...
{
	int i = 0;
	int led[] = {POWER_LED, WAN_LED, WLAN_LED};

	for (i = 0; i < ARRAY_SIZE(led); i++) {
		mtk_set_gpio_dir(led[i], GPIO_DIR_OUTPUT);
	}
}

void ALL_LED_STATUS(enum led_status status)
{
	int i = 0;
	int led[] = {POWER_LED, WAN_LED, WLAN_LED};

	for (i = 0; i < ARRAY_SIZE(led); i++) {
		mtk_set_gpio_pin(led[i], status);
	}
}

void LED_STATUS(enum led_gpio led, enum led_status status)
{
	return mtk_set_gpio_pin(led, status);
} 

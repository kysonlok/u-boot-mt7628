...
#if defined(MT7628_ASIC_BOARD)	/* Disable WLED share pin */
	RALINK_REG(RALINK_SYSCTL_BASE+0x3C)|= (1<<8);	
	/* RALINK_REG(RALINK_SYSCTL_BASE+0x64)&= ~((0x3<<16)|(0x3)); */
	RALINK_REG(RALINK_SYSCTL_BASE+0x64)&= ~((0x3<<16)|(0x3));
	RALINK_REG(RALINK_SYSCTL_BASE+0x64)|= ((0x01<<16)|(0x01));
#endif	
...
/*config bootdelay via environment parameter: bootdelay */
	...
	for ( led_nr = 30; led_nr <= 95; led_nr++ ) {

		printf( "LED nr: %d on   ", led_nr );
		mtk7628_set_gpio_dir( led_nr, 0);
		mtk7628_set_gpio_pin( led_nr, 0 );
		udelay( 2000000 );

		printf( "LED nr: %d off\n", led_nr );
		mtk7628_set_gpio_dir( led_nr, 0);
		mtk7628_set_gpio_pin( led_nr, 1 );
		udelay( 1000000 );
	}
#endif
